# Security Policy
Pull Requests with unneccesary modifications in system commands like `renice` will not be accepted unless important.
## Supported Versions

v3.2.0 and above supports pull requests as of 6th April 2026. All other pull requests for lower versions will not be accepted

| Version | Supported          |
| ------- | ------------------ |
| 3.1.0   | ❎                 |
| 3.2.0   | ✅                 |
| 3.3.0   | ✅                 |


## Reporting a Vulnerability

Feel free to report vulnerabilities 🙂.

# Push Requests life cycle:
After a new release is releasesd only that release and the previous release will be supported for push request. Any older versions will be deemed outdated!
